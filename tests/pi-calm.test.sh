...
# - static wiring: bootstrap.sh symlink auto-load, TypeScript typecheck, JS syntax;
...
test_zero_coupling_and_state_file() {
  ...
  for file in $source_files "$ROOT/tests/pi-calm.test.sh" "$ROOT/tests/lib.sh" "$ROOT/README.md" "$ROOT/bootstrap.sh"; do
  ...
  license_hits=$(grep -rni "$attribution_name" "$CALM_DIR" "$ROOT/README.md" "$ROOT/bootstrap.sh" 2>/dev/null | grep -v "Adapted from" || true)
  ...
  # The runtime preference file must never be tracked or symlinked by bootstrap.
  ...
  assert_not_contains "$(cat "$ROOT/bootstrap.sh")" '.pi/agent/calm' "bootstrap.sh manages the Calm state file"
...
test_static_typescript_and_repo_wiring() {
  # bootstrap.sh links the extensions directory as a whole, so the calm
  # subdirectory auto-loads without any new declaration.
  grep -q 'ln -sfn ~/.dotfiles/home/.pi/agent/extensions ~/.pi/agent/extensions' "$ROOT/bootstrap.sh" \
    || fail "bootstrap.sh no longer links ~/.pi/agent/extensions as a directory"
...
JSON
  ...
  pass "static wiring: bootstrap.sh symlink auto-load intact, TypeScript typechecks, existing JS extension parses"
}
